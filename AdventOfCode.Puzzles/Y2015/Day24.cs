...
[AocPuzzle(2015, 24, "It Hangs in the Balance")]
public class Day24 : IDay<long>
{
    public long Part1(ReadOnlySpan<char> span) // 1 min
    {
        ...
        for (int i = IndexWhneGreater(dataSpan, av); i < 1 << dataSpan.Length; i++)
        {
            ...
            if (sum == av)
            {
                ...
                Int128 qe = default;
                if ((candidate.Count == 0 || candidate.Count > c.Count || QE > (qe = c.AsReadOnlySpan().Aggregate(Int128.One, (p, c) => p * c))) && Calculate(CollectionsMarshal.AsSpan(testData), av))
                {
                    (candidate, c) = (c, candidate);
                    QE = qe == default ? c.AsReadOnlySpan().Aggregate(Int128.One, (p, c) => p * c) : qe;
                }
            }
        Next:;
        }
