...
public class Day06 : IDay<int>
{
    public int Part1(ReadOnlySpan<char> span)
    {
        ...
        {
            ParseInput(line, out var x1, out var y1, out var x2, out var y2);
            while (x1++ <= x2)
            {
                for (var y3 = y1; y3 <= y2; y3++)
                {
                    grid[y3, x1] = line[6] != 'f' && (line[6] == 'n' || !grid[y3, x1]);
                    //grid[y3, x1] = item[6] == 'f' ? false : item[6] == 'n' ? true : !grid[y3, x1];
                }
            }
        }
        return grid.AsEnumerable().Count(static x => x);
    }
