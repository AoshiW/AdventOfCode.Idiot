...
public class Day16 : IDay<int>
{
    ...
    static Aunt ParseLine(ReadOnlySpan<char> span)
    {
        ...
        var enumerator = span.SplitAny(" :,");
        while (enumerator.MoveNext())
        {
            while (span[enumerator.Current].IsEmpty)
                enumerator.MoveNext();
            var key = span[enumerator.Current];
            enumerator.MoveNext();
            while (span[enumerator.Current].IsEmpty)
                enumerator.MoveNext();
            var value = int.Parse(span[enumerator.Current]);
    ...
    struct Aunt
    {
        public int? Children, Cats, Trees, Pomeranians, Goldfish, Samoyeds, Akitas, Vizslas, Cars, Perfumes;
        public int ID;
    }
}
