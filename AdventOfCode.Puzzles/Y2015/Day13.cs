...
public class Day13 : IDay<int>
{
    public int Part1(ReadOnlySpan<char> span)
        => TotalChangeInHappiness(span, false);

    public int Part2(ReadOnlySpan<char> span)
        => TotalChangeInHappiness(span, true);
    ...
    static int TotalChangeInHappiness(ReadOnlySpan<char> span, bool addMe)
    {
        ...
        {
            var from = line.Slice(0, line.IndexOf(' ')).ToString();
            var to = line.Slice(line.LastIndexOf(" ") + 1).TrimEnd('.').ToString();
            var value = line.Contains(" lose ", StringComparison.OrdinalIgnoreCase) ? -1 : 1;
            var item2 = line.Slice(0, line.IndexOf(" happiness "));
